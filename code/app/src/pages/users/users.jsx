...
export default function Users(props) {
    ...
    function getUsers() {
        window.api.get('asynchronous-get','user').then(result => {
            updateUser(result)
        })
    }
                ...
                <div className="component">
                    <div className="awaiting">
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {usersa && usersa.map((user, idx) => {
                                    return <tr key={'users_awaiting_list_'+idx}>
                                        <td key={'users_awaiting_a'+idx}>
                                            <input key={'users_awaiting_check_'+idx} type="checkbox" checked={awaiting[idx]} 
                    ...
                    <div className="validated">
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {usersv && usersv.map((user, idx) => {
                                    return <tr key={'users_validated_list'+idx}>
                                        <td key={'users_validated_a'+idx}>
                                            <input key={'users_validated_check'+idx} type="checkbox" checked={validated[idx]} 
                                                onChange={()=>handleOnChange(true, idx)}
                                            />
                                        </td>
                                        <td key={'users_validated_n'+idx}>
                                            {user.nom}
                                        </td>
                                        <td key={'users_validated_e'+idx}>
                                            {user.email}
                                        </td>
                                        <td key={'users_validated_t'+idx}>
                                            {user.tel}
                                        </td>
                                        <td key={'users_validated_l'+idx}>
                                            {user.label}
                                        </td>
                                    </tr>
                                })}
